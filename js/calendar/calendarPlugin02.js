...
var calendar = {
  ...
  createTabledate: function(year, month) {
    ...
    for (var i = lastdays; i <= pervMonthlastDay; i++) {
      ...
      var a = calendar.getA(parseInt(month, 10) - 1 == 0 ? parseInt(year) - 1 : year, parseInt(month, 10) - 1 == 0 ? 12 : parseInt(month, 10) - 1, i);
      //上个月最后几天的样式
    ...
    for (var i = 0; i < loopT; i++) {
      ...
      for (var n = 1; n <= 7; n++) {
        ...
        if (startDays == 0) {
          //下个月
          var c = calendar.getA(parseInt(month, 10) + 1 == parseInt(13) ? parseInt(year) + 1 : year, parseInt(month, 10) + 1 == parseInt(13) ? 1 : parseInt(month, 10) + 1, lastFirstDay);
          //下个月最后几天的样式
